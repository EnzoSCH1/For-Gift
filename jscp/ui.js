...
let matrixChars = "ILOVEYOU".split("");
...
// La liste photoUrls sera initialisée dynamiquement dans spawnHeartPhotosCentered
...
function createHeartPhotoCentered(idx, total, photoUrls) {
...
function spawnHeartPhotosCentered() {
    ...
    // (Ré)initialise photoUrls à chaque appel pour prendre la valeur courante de window.heartImages
    let photoUrls = (window.heartImages && window.heartImages.length > 0)
        ? window.heartImages
    ...
    function spawnNext() {
        if (currentIndex < maxHeartPhotos) {
            createHeartPhotoCentered(currentIndex, maxHeartPhotos, photoUrls);
            currentIndex++;
...
// Met à jour dynamiquement la source audio selon settings.music
if (birthdayAudio && window.settings && window.settings.music) {
